...
class MSSA(nn.Module):
    """
    White-Box Transformer -- Multi-Head Subspace Self-Attention
    """
...
class ISTA(nn.Module):
    """
    White-Box Transformer -- Iterative Shrinkage Thresholding Algorithm
    """
...
class DiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, **block_kwargs):
        ...
        self.attn = MSSA(dim=hidden_size, heads=num_heads)
        self.norm2 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        mlp_hidden_dim = int(hidden_size * mlp_ratio)
        self.mlp = ISTA(dim=hidden_size, hidden_dim=mlp_hidden_dim, dropout=0.)
...
class WBDiT(nn.Module):
    ...
    def unpatchify(self, x):
        ...
        # x: [16, 200, 10]
        c = self.out_channels
        ph = self.x_embedder.patch_size[0]
        pw = 1
        h = x.shape[1]
        w = 1
        ...
        imgs = x.reshape(shape=(x.shape[0], c, h * ph))
        return imgs
